<flux:sidebar sticky collapsible class="bg-zinc-50 dark:bg-zinc-900 border-r border-zinc-200 dark:border-zinc-700">
    ...
    <flux:sidebar.nav>
        ...
        <flux:sidebar.group expandable heading="{{ __('messages.tenants') }}" :expanded="checkGroupExpanded('tenants')">
            <flux:sidebar.item icon="list-bullet" href="{{ route('tenants.index') }}"
                               :current="Route::currentRouteName() === 'tenants.index'">
                {{ __('messages.list') }}
            </flux:sidebar.item>
            <flux:sidebar.item icon="plus" href="{{ route('tenants.create') }}"
