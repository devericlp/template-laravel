<div>
    ...
    <x-datatable :headers="$this->headers" :items="$this->items" :page-lengths="$pageLengths" :per-page="$perPage" :search="$search"
        :sort-by="$sortBy" :sort-direction="$sortDirection" selectable>
        <x-slot name="bulkActions">
            <flux:navmenu.item class="cursor-pointer"
                @click="$dispatch('confirm-delete-selected-users', { selected: selected })" icon="trash">
                {{ __('messages.delete_selected') }}</flux:navmenu.item>
            <flux:navmenu.item class="cursor-pointer"
                @click="$dispatch('confirm-activate-selected-users', { selected: selected })" icon="check">
                {{ __('messages.activate_selected') }}</flux:navmenu.item>
            <flux:navmenu.item class="cursor-pointer"
                @click="$dispatch('confirm-inactivate-selected-users', { selected: selected })" icon="x-mark">
                {{ __('messages.inactivate_selected') }}</flux:navmenu.item>

            @if (!is_null($this->filters['visibility']))
                <flux:navmenu.item class="cursor-pointer"
                    @click="$dispatch('confirm-restore-selected-users', { selected: selected })" icon="arrow-path">
                    {{ __('messages.restore_selected') }}</flux:navmenu.item>
            @endif
        </x-slot>

        <x-slot name="fields">
            <flux:select wire:model="filters.visibility" label="{{ __('messages.visibility') }}"
                placeholder="{{ __('messages.all') }}" variant="listbox" clearable>
                @foreach ($visibilities as $visibility)
                    <flux:select.option value="{{ $visibility['id'] }}">{{ $visibility['name'] }}</flux:select.option>
                @endforeach
            </flux:select>
            <flux:select wire:model="filters.role_id" label="{{ __('messages.role') }}"
                placeholder="{{ __('messages.all') }}" variant="listbox" clearable>
        ...
        @scope('cell_actions', $user)
            <flux:button.group>
                <flux:button href="{{ route('users.show', ['user' => $user]) }}" class="cursor-pointer"
                    tooltip="{{ __('messages.show') }}" icon:variant="outline" icon="eye" size="sm" />
                <flux:button href="{{ route('users.update', ['user' => $user]) }}" class="cursor-pointer"
                    tooltip="{{ __('messages.edit') }}" icon:variant="outline" icon="pencil" size="sm" />

                <flux:button @click="$dispatch('confirm-impersonate-user', { userId: {{ $user->id }} })"
                    class="cursor-pointer" tooltip="{{ __('messages.impersonate') }}" icon:variant="outline"
                    icon="hat-glasses" size="sm" />
                ...
                @else
                    <flux:button @click="$dispatch('confirm-restore-user', { userId: {{ $user->id }} })"
                        class="cursor-pointer" tooltip="{{ __('messages.restore') }}" icon:variant="outline" icon="arrow-path"
                        size="sm" />
                @endunless
            </flux:button.group>
        @endscope
