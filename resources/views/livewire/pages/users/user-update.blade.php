<div>
    <x-loading />
    <x-page-header :title="__('messages.update_user', ['name' => $user->name])" :back="Str::contains(url()->previous(), route('users.index'))" back-title="{{ __('messages.back') }}" />
        ...
        <div class="grid grid-cols-12 gap-5">
            ...
            <div class="col-span-12 md:col-span-3 order-1 md:order-2">
                <x-image-upload model="avatar" removeAction="removeAvatar"
                    initial-image="{{ $user->avatar ? asset($user->avatar) : null }}" />
            </div>
        </div>
    </x-form>
</div>
