<div>
    <x-loading/>
    <x-page-header
        :title="__('messages.create_tenant')"
        :back="Str::contains(url()->previous(), route('tenants.index'))"
                        ...
                        <flux:accordion.item expanded>
                            <flux:accordion.heading>
                                ...
                                @if(
                                    ...
                                    <flux:badge class="ml-1" size="sm" color="orange"> {{ __('messages.Incomplete') }}</flux:badge>
                                @endif
                            </flux:accordion.heading>
                            <flux:accordion.content>
                                <flux:fieldset class="pt-5">
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <flux:field>
                                            <flux:label
                                                badge="{{ __('messages.required') }}">{{ __('messages.identification_number') }}</flux:label>
                                        ...
                                        <flux:field>
                                            <flux:label
                                                badge="{{ __('messages.required') }}">{{ __('messages.social_reason') }}</flux:label>
                                        ...
                                        <flux:field>
                                            <flux:label
                                                badge="{{ __('messages.required') }}">{{ __('messages.email') }}</flux:label>
                                        ...
                                        <flux:field>
                                            <flux:label
                                                badge="{{ __('messages.required') }}">{{ __('messages.cellphone') }}</flux:label>
                                    ...
                                    <flux:field class="w-full mt-5">
                                        <flux:label
                                            badge="{{ __('messages.required') }}">{{ __('messages.subdomain') }}</flux:label>
                                        ...
                                        <flux:input.group>
                                            ...
                                            <flux:input.group.suffix>
                                                @php
                                                    ...
                                                    $host = $parts_url['host'] . (isset($parts_url['port']) ? ':' . $parts_url['port'] : '');
                                                @endphp
                                                .{{ $host }}
                                            </flux:input.group.suffix>
                                        </flux:input.group>
                                ...
                                <flux:fieldset class="pt-5">
                                    ...
                                    <div class="flex flex-col md:flex-row gap-4">
                                        <flux:field class="w-full md:w-56">
                                            <flux:label
                                                badge="{{ __('messages.required') }}">{{ __('messages.zipcode') }}</flux:label>
                                            <flux:input.group>
                                        ...
                                        <flux:field class="w-full md:flex-1">
                                            <flux:label
                                                badge="{{ __('messages.required') }}">{{ __('messages.street') }}</flux:label>
                                        ...
                                        <flux:field class="">
                                            ...
                                            <flux:input wire:model="neighborhood" size="sm" icon="building-office-2"
                                        ...
                                        <flux:field class="sm:w-full md:w-56">
                                            ...
                                            >
                                                @foreach($countries as $country)
                                                    <flux:autocomplete.item>{{ $country['name'] }}</flux:autocomplete.item>
                                                @endforeach
                                            </flux:autocomplete>
                                        </flux:field>
                                        <flux:field class="w-full md:flex-1">
                                            <flux:label>{{ __('messages.complement') }}</flux:label>
                                            <flux:input wire:model="complement" size="sm" icon="information-circle"
                ...
                <flux:card class="w-full flex flex-col justify-center space-y-3 items-center">
                    @if ($logo)
                        <div class="relative inline-block">
                            <img src="{{ $logo->temporaryUrl() }}" class="rounded-full size-32"
                                 alt="preview">
